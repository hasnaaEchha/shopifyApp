...
!(function (g) {
    g.extend({
        tablesorter: new function () {
            ...
            function x(a) {
                ...
                if (d[0])for (h = [], y = d[0].cells.length, f = 0; f < y; f++) {
                    ...
                    if (!g)a:{
                        ...
                        r = d;
                        g = -1;
                        for (var A = f, l = void 0, m = e.parsers.length, t = !1, v = "", l = !0; "" === v && l;)g++, r[g] ? (t = r[g].cells[A], v = p(k, t, A), k.config.debug && c("Checking if value was empty on row " + g + ", column: " + A + ': "' + v + '"')) : l = !1;
                        for (; 0 <= --m;)if ((l = e.parsers[m]) && "text" !== l.id && l.is && l.is(v, k, t)) {
                            g = l;
                            break a
                        }
            ...
            function z(a) {
                ...
                for (n = 0; n < b.length; n++)if (d.cache[n] = {
                        row: [],
                        normalized: []
                    ...
                    for (r = 0; r < h; ++r)if (A = g(b[n].rows[r]), m = [], A.hasClass(d.cssChildRow))d.cache[n].row[d.cache[n].row.length - 1] = d.cache[n].row[d.cache[n].row.length - 1].add(A); else {
                        ...
                        for (q = 0; q < y; ++q)k = p(a, A[0].cells[q], q), k = f[q].format(k, a, A[0].cells[q], q), m.push(k), "numeric" === (f[q].type || "").toLowerCase() && (t[q] = Math.max(Math.abs(k) || 0, t[q] || 0));
            ...
            function B(a, b) {
                ...
                if (!m(f)) {
                    d.debug && (u = new Date);
                    for (t = 0; t < h.length; t++)if (k = g(h[t]), k.length && !k.hasClass(d.cssInfoBlock)) {
                        A = e.processTbody(a, k, !0);
                        k = f[t].row;
                        r = f[t].normalized;
                        n = (q = r.length) ? r[0].length - 1 : 0;
                        for (p = 0; p < q; p++)if (s = r[p][n], c.push(k[s]), !d.appender || !d.removeRows)for (v = k[s].length, w = 0; w < v; w++)A.append(k[s][w]);
            ...
            function E(a) {
                ...
                for (f = 0; f < c.length; f++)for (n = c[f].cells, k = 0; k < n.length; k++) {
                    q = n[k];
                    ...
                    for (e = 0; e < b[l].length + 1; e++)if ("undefined" === typeof b[l][e]) {
                        v = e;
                        break
                    }
                    d[m] = v;
                    ...
                    for (e = l; e < l + p; e++)for ("undefined" === typeof b[e] && (b[e] = []), m = b[e], q = v; q < v + t; q++)m[q] = "x"
                }
            ...
            function H(a) {
                ...
                h = f.length;
                for (b = 0; b < h; b++)if (2 !== f[b][1] && (a = c.$headers.not(".sorter-false").filter('[data-column="' + f[b][0] + '"]' + (1 === h ? ":last" : "")), a.length))for (d = 0; d < a.length; d++)a[d].sortDisabled || (a.eq(d).addClass(k[f[b][1]]), r.length && r.filter('[data-column="' + f[b][0] + '"]').eq(d).addClass(k[f[b][1]]))
            }
            ...
            function I(a) {
                ...
                if (!s.serverSideSorting && !m(s.cache)) {
                    s.debug && (p = new Date);
                    for (d = 0; d < C; d++)k = s.cache[d].colMax, n = (g = s.cache[d].normalized) && g[0] ? g[0].length - 1 : 0, g.sort(function (d, g) {
                        for (b = 0; b < B; b++) {
                            f = z[b][0];
                            q = z[b][1];
                            w = (v = 0 === q) ? d : g;
            ...
            e.refreshWidgets = function (a, b, d) {
                ...
                for (h = 0; h < m; h++)k[h] && k[h].id && (b || 0 > g.inArray(k[h].id, f)) && (l.debug && c("Refeshing widgets: Removing " + k[h].id), k[h].hasOwnProperty("remove") && (k[h].remove(a, l, l.widgetOptions), l.widgetInit[k[h].id] = !1));
    ...
    p.addWidget({
        id: "zebra", priority: 90, format: function (c, l, m) {
            ...
            for (m = 0; m < l.length; m++)p = g.tablesorter.processTbody(c, l.eq(m), !0), p.children().removeClass(x), g.tablesorter.processTbody(c, p, !1)
        }
    })
})(jQuery);
